...
const fallbackResponses = {
  rw: {
    pregnancy:
      "Ubusanzwe ubuzima bwawe bwiza. Reba ko ufata vitamini zawe buri munsi kandi ujya kwa muganga. Umwana wawe akura neza.",
    emergency:
      "Ibi ni ibibazo by'ingenzi! Ijya kwa muganga vuba cyane cyane niba ufite amaraso cyangwa ububabare. Ntuzagire ubwoba.",
    nutrition:
      "Fata ibiryo byuzuye amatungo, imboga, n'imbuto. Reba ko unywa amazi menshi. Ibi bizagufasha kuba ufite ubuzima bwiza.",
    mental_health:
      "Ubuzima bwawe bw'ubwenge ni ngombwa. Vuga ibibazo byawe kwa muganga cyangwa umufasha. Ntuzagire ubwoba.",
    exercise:
      "Gukora sport bizagufasha kuba ufite ubuzima bwiza. Reba ko ukora sport yoroshye nka kugenda.",
    default:
      "Nshobora kugufasha kugenzura ubuzima bwawe. Vuga ibibazo byawe cyangwa ibyo ushaka kumenya.",
  },
  en: {
    pregnancy:
      "Your pregnancy is progressing well. Make sure to take your vitamins daily, attend regular checkups, and get plenty of rest.",
    emergency:
      "These are emergency symptoms! Go to the hospital immediately, especially if you have bleeding or severe pain. Don't delay.",
    nutrition:
      "Eat a balanced diet with proteins, vegetables, and fruits. Make sure to drink plenty of water. This will help you stay healthy.",
    mental_health:
      "Your mental health is important. Talk to your doctor or counselor about your concerns. Don't be afraid to seek help.",
    exercise:
      "Regular exercise will help you stay healthy. Try gentle activities like walking or prenatal yoga.",
    default:
      "I can help you monitor your health. Tell me your concerns or what you want to know.",
  },
};
...
async function findIndicatorCodeByKeywords(keywords = []) {
  ...
  const url = `${WHO_BASE}/Indicator?$select=IndicatorCode,IndicatorName&$filter=${encodeURI(
    q
  )}&$top=100`;
  ...
  const best = list.sort(
    (a, b) => b.IndicatorName.length - a.IndicatorName.length
  )[0];
  if (!best) return null;
...
const INTENT_MAP = {
  anc4: {
    ...
    display: {
      rw: "Gusuzumwa kuwa mbere yo kubyara (inshuro 4+ %)",
      en: "Antenatal care (4+ visits %)",
    },
  },
  sba: {
    keywords: [
    ...
    display: {
      rw: "Kubyara byitabiriwe n'abaganga b'inzobere (%)",
      en: "Births attended by skilled health personnel (%)",
    },
  },
  mmr: {
    keywords: ["maternal mortality ratio", "mmr", "maternal deaths per 100000"],
    display: {
      rw: "Igipimo cy'urupfu rw'ababyeyi (ku 100,000)",
      en: "Maternal mortality ratio (per 100,000 live births)",
    },
  },
  abr: {
...
function detectIntents(message) {
  ...
  if (
    intents.length === 0 &&
    /(pregnan|maternal|umubyeyi|kwitera|kwabyara|kwisuzumisha)/i.test(lower)
  ) {
    return ["anc4", "sba", "mmr"];
  }
...
function renderAnswer(results = [], lang = "rw") {
  ...
  const lines = results.map((r) => {
    const title = Object.values(INTENT_MAP).find(
      (x) => x.display.en === r.intentDisplay.en
    )
      ? r.intentDisplay[lang]
      : r.indicatorName;
...
async function answerMaternalQuestion({
  message,
  language = "rw",
  country = "RWA",
}) {
...
async function fetchFromSources(category) {
        ...
        if (url.includes("ghoapi.azureedge.net")) {
          if (resp.data.value && Array.isArray(resp.data.value)) {
            ...
            if (latestData.length > 0) {
              const avgValue =
                latestData.reduce((sum, item) => sum + item.Value, 0) /
                latestData.length;
              ...
              successCount++;
              console.log(
                `✅ Successfully fetched WHO data: ${resp.data.value[0]?.Indicator}`
              );
            }
          }
  ...
  console.log(
    `📊 Fetched ${successCount} successful responses for ${category}`
  );
...
function generateSummaryFromData(data, category) {
  ...
  if (category === "pregnancy") {
    if (whoData.length > 0) {
      const ancCoverage = whoData.find((item) =>
        item.indicator.includes("ANC4")
      );
      const skilledBirth = whoData.find((item) =>
        item.indicator.includes("SKILLED")
      );
      const maternalMortality = whoData.find((item) =>
        item.indicator.includes("MATERNAL_MORTALITY")
      );
      ...
      if (ancCoverage) {
        const trend =
          ancCoverage.trend === "improving"
            ? "improving"
            : ancCoverage.trend === "declining"
            ? "needs attention"
            : "stable";
        summary += `Antenatal care coverage is around ${ancCoverage.value.toFixed(
          1
        )}${ancCoverage.unit} and is ${trend}. `;
      }

      if (skilledBirth) {
        const trend =
          skilledBirth.trend === "improving"
            ? "improving"
            : skilledBirth.trend === "declining"
            ? "needs attention"
            : "stable";
        summary += `Skilled birth attendance rate is approximately ${skilledBirth.value.toFixed(
          1
        )}${skilledBirth.unit} and is ${trend}. `;
      }
  ...
  if (category === "emergency") {
    if (whoData.length > 0) {
      const emergencyCare = whoData.find((item) =>
        item.indicator.includes("EMERGENCY")
      );
  ...
  if (category === "nutrition") {
    if (whoData.length > 0) {
      const nutritionData = whoData.find(
        (item) =>
          item.indicator.includes("NUTRITION") ||
          item.indicator.includes("OVERWEIGHT")
      );
  ...
  if (category === "mental_health") {
    if (whoData.length > 0) {
      const mentalHealthData = whoData.find((item) =>
        item.indicator.includes("MENTAL")
      );
  ...
  if (whoData.length > 0) {
    ...
    return `Recent WHO data shows ${
      latestData.indicator
    } at ${latestData.value.toFixed(1)}${latestData.unit} with ${
      latestData.trend
    } trends. This information helps guide health decisions.`;
  }
...
if (process.env.MONGODB_URI) {
  mongoose
    ...
    .catch((err) => {
      ...
      console.log(
        "💡 To enable database features, chaeck you internet connection and reconnect to the internet"
      );
    });
}
...
app.get("/api/who/maternal", async (req, res) => {
  try {
    ...
    const result = await answerMaternalQuestion({
    ...
    res
      .status(500)
      .json({
        error: "Failed to fetch WHO maternal data",
        details: err.message,
      });
  }
});
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    // Use the correct version string from Replicate's API tab!
    ...
    // Replicate's response may require polling for completion
    let aiText = "";
    if (replicateResponse.data && replicateResponse.data.output) {
      aiText = Array.isArray(replicateResponse.data.output)
        ? replicateResponse.data.output.join(" ")
        : replicateResponse.data.output;
    } else {
      aiText = "Sorry, I couldn't get a response from the AI model.";
    }
    ...
    res.status(500).json({
      type: "bot",
      content:
        "Sorry, I couldn’t process that. Please ask again or consult a doctor if urgent.",
      source: "fallback",
    });
  }
});
...
function translateToKinyarwanda(englishText) {
  const translations = {
    // WHO Data references
    "Based on recent WHO global health data":
    "Hashingiye ku makuru ya WHO ya vuba aha y'ubuzima bw'isi yose",
    "Based on recent WHO data": "Hashingiye ku makuru ya WHO ya vuba aha",

    // Pregnancy indicators
    "Antenatal care coverage is around":
      "Ubusanzwe bwo kujya kwa muganga mbere yo kubyara buri hagati ya",
    "Skilled birth attendance rate is approximately":
      "Ubusanzwe bwo kubyara kwa muganga buri hagati ya",
    "Maternal mortality trends show":
      "Ibimenyetso by'urupfu rw'ababyeyi birekana",
    ...
    stable: "Bihagaze Neza",
    declining: "biragenda nabi",
    "needs attention": "bikenewe kwitabwaho",

    // Health outcomes
    "These indicators help track global maternal health progress":
      "Ibi bimenyetso bifasha kugenzura iterambere ry'ubuzima bw'ababyeyi ku isi yose",
    "These are positive indicators for maternal health outcomes":
      "Ibi ni ibimenyetso byiza by'ubuzima bw'ababyeyi",

    // Emergency care
    "Emergency care access shows": "Kubona ubufasha bw'ingenzi birekana",
    "trends globally": "ibimenyetso ku isi yose",
    "It's crucial to have emergency care available during pregnancy":
      "Ni ngombwa kuba ufite ubufasha bw'ingenzi mbere yo kubyara",

    // Nutrition
    "Nutrition indicators show": "Ibimenyetso by'ibiryo birekana",
    "Proper nutrition during pregnancy is essential for both mother and baby":
      "Ibiryo byiza mbere yo kubyara ni ngombwa kuri nyina n'umwana",

    // Mental health
    "Mental health awareness is": "Kumenya ubuzima bw'ubwenge ni",
    "It's important to address mental health during pregnancy and postpartum":
      "Ni ngombwa kugenzura ubuzima bw'ubwenge mbere yo kubyara n'nyuma yo kubyara",

    // General
    "Recent health data shows": "Makuru ya vuba y'ubuzima yerekana",
    "positive trends in maternal care access and outcomes":
      "ibimenyetso byiza by'ubuzima bw'ababyeyi",
    "This information helps guide health decisions":
      "Iyi makuru ifasha gufata ibyemezo by'ubuzima",
